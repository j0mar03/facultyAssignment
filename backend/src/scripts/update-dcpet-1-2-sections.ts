...
export const updateDCPET12Sections = async () => {
  try {
    ...
    console.log('Removing existing DCPET 1-2 sections...');

    // First get the existing DCPET 1-2 sections
    ...
    console.log('Creating new DCPET 1-2 sections...');
    ...
    for (const courseCode of uniqueCourses) {
      ...
      // Find the course in database (these should already exist from DCPET 1-1 creation)
      ...
      const section = sectionRepo.create({
        ...
        notes: `Updated schedule for ${courseCode} in DCPET 1-2`
      });
    ...
    console.log('DCPET 1-2 schedule update completed successfully!');
    console.log(`Total sections created: ${uniqueCourses.length}`);
    
  } catch (error) {
    console.error('Error updating DCPET 1-2 sections:', error);
  } finally {
